...
class Eh_PayPal_Express_Payment extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->smart_button_environment = $this->get_option('smart_button_environment');
        ...
        $this->save_abandoned_order = $this->get_option('smart_button_save_cancelled_order') === "yes" ? true : false;
        $this->save_abandoned_order_express = $this->get_option('save_cancelled_order') === "yes" ? true : false;
        ...
        $this->payment_mode = $this->get_option('smart_button_enabled');
            if ($this->payment_mode == 'yes') {
                if ($this->smart_button_environment === 'sandbox') { 
    ...
    public function process_admin_options() {
        ...
        if($this->payment_mode == 'yes'){
            if(get_transient('eh_access_token') !== false){
    ...
    public function perform_api_request_paypal() {
        ...
        switch ($action_code) {
                ...
                try {
                    ...
                    $request_params = $this->new_request()->make_request_params
                            ...
                            array
                                ...
                                'return_url' => add_query_arg(array('express' => $express, 'p'=> $page_referer, 'pay_for_order' => $pay_for_order), eh_paypal_express_run()->hook_include->make_express_url('express_details')),
            ...
            case 'express_details':
                ...
                if ((!isset($_REQUEST['pay_for_order']) || !$_REQUEST['pay_for_order']) &&  !$this->save_abandoned_order_express) {
                    $order_id = $this->create_wc_order($checkout_post);
                    $order = wc_get_order( $order_id );
            ...
            case 'create_order': 
                ...
                try {
                    ...
                    $return_url = add_query_arg(array('p' => $page_referer, 'intent' => $intent, 'pay_for_order' => $pay_for_order), eh_paypal_express_run()->hook_include->make_express_url('order_details'));
            ...
            case 'order_details': 
                //create order only if save abondoned orderis disabled and is not an order pay page
                if ((!isset($_REQUEST['pay_for_order']) ||  !$_REQUEST['pay_for_order']) &&  !$this->save_abandoned_order) {
                    $order_id = $this->create_wc_order($checkout_post);
                    $order = wc_get_order( $order_id );
...
/*                        }
    ...
    public function add_cancel_order_elements() {
        if (isset(WC()->session->eh_pe_checkout)) {
            ...
            $page = get_page_link();
            printf('<a href="' . add_query_arg('p', $page, eh_paypal_express_run()->hook_include->make_express_url('cancel_express')) . '" class="button alt" style="background-color: crimson;margin-top: %s;">'.__( 'Cancel Order','express-checkout-paypal-payment-gateway-for-woocommerce').'</a>', "5%");
        }
    }
    ...
    */
    ...
    public function shipping_parse($response, $type = null) {
        ...
        if ($type == 'rest') {
            ...
            if (isset($response['ADDRESSSTATUS']) && isset($response['FIRSTNAME'])) {
                $shipping_details = array
                    (
                    'first_name' => (($this->paypal_allow_override  && isset($post_data['billing_first_name']) && !empty($post_data['billing_first_name']))? $post_data['billing_first_name'] : $name_first),
                    'last_name' => (($this->paypal_allow_override  && isset($post_data['billing_last_name']) && !empty($post_data['billing_last_name'])) ? $post_data['billing_last_name'] : ( isset($name_middle) ? ($name_middle.' '.(isset($name_last) ? $name_last: ' ')) : ' ' )),
    ...
    public function eh_error_msg_processing($response, $method){
        ...
        if($method == 'DoExpressCheckoutPayment' && isset($response['L_ERRORCODE0']) && ($response['L_ERRORCODE0'] == 10486) ){
            wp_redirect($this->make_redirect_url($response['TOKEN']));
        }
        else{         
